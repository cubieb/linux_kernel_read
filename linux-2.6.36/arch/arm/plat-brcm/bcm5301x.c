...
#ifdef GPIO_INTERRUPT_EXAMPLE
#include "siutils.h"
#define GPIO_INT_IRQ (117)
#define GPIO_INTR_PIN (17)

si_t *gpiointr_sih = NULL;
uint32 fake_devid=0;
...
void reset_wq_func();
void gpio_edge_interrupt_init(void);

void reset_wq_func()
{
	while(system_boot)
        {
                val = (1 << GPIO_INTR_PIN) & si_gpioin(gpiointr_sih);

                if (val == 0) {
                        cnt++;
                        if (cnt > 50)   //more than 5 seconds
                        {
                                cnt = 0;

                                //turn on yellow led
				si_gpioout(gpiointr_sih, 1<<3, 1, GPIO_DRV_PRIORITY);
				si_gpioout(gpiointr_sih, 1<<1, 1, GPIO_DRV_PRIORITY);
				si_gpioout(gpiointr_sih, 1<<2, 0, GPIO_DRV_PRIORITY);

                                //Restore factory settings
                                const char * restore_defaults = "restore_defaults";
                        ...
                        if (cnt) {
                                if (cnt < 20)   //less than 2 seconds
                                {
                                        cnt = 0;

                                        // Soft-shutdown function
					if (nvram_match("flag_package_update", "7"))
					{
						//Ignore this action
					}
					else
					{
						si_gpioout(gpiointr_sih, 1<<1, 1, GPIO_DRV_PRIORITY);
						si_gpioout(gpiointr_sih, 1<<2, 1, GPIO_DRV_PRIORITY);
						si_gpioout(gpiointr_sih, 1<<3, 1, GPIO_DRV_PRIORITY);

						const char * flag_package_update = "flag_package_update";
						nvram_set(flag_package_update, "6");
						kernel_restart(NULL);
					}
...
static irqreturn_t
gpio_edge_isr(int irq, void *dev_id)
{
       uint32 gpioevent;

       /* get gpio event # to confirm this is our interrupt */
       gpioevent = si_gpioevent(gpiointr_sih, GPIO_REGEVT, (0), (0));

       //printk(KERN_ERR "\n gpio_edge_isr: triggerred gpioevent 0x%08x\n", gpioevent);

       si_gpioevent(gpiointr_sih, GPIO_REGEVT, (1<<GPIO_INTR_PIN), (1<<GPIO_INTR_PIN));

       if(gpioevent& (1<<GPIO_INTR_PIN)) {
		/* this is our interrupt. handle it */
		schedule_work(&reset_wq);
       }

       //IRQ_HANDLED
       return IRQ_RETVAL(gpioevent & (1<<GPIO_INTR_PIN));
}

void gpio_edge_interrupt_init()
{
	gpiointr_sih= si_kattach(SI_OSH);

	if (request_irq(GPIO_INT_IRQ, gpio_edge_isr, IRQF_SHARED, "gpioint", &fake_devid)) {
		printk(KERN_ERR "\n error registering for irq gpiointr \n");
		return;
	}

	INIT_WORK(&reset_wq, (void (*) (void *))reset_wq_func);

	//Set Failing Edge
	si_gpioevent(gpiointr_sih, GPIO_REGEVT_INTPOL, (1<<GPIO_INTR_PIN), (1<<GPIO_INTR_PIN));

	/* enable gpioevent for edge level interrupt */
	si_gpioevent(gpiointr_sih, GPIO_REGEVT_INTMSK, (1<<GPIO_INTR_PIN), (1<<GPIO_INTR_PIN));

	/* change edge polarity if needed. */
	//si_gpioevent(gpiointr_sih, GPIO_REGEVT_INTPOL, (1<<GPIO_INTR_PIN), (1<<GPIO_INTR_PIN));

	/* enable gpio interrupt */
	si_gpio_int_enable(gpiointr_sih, 1);
}
#endif
