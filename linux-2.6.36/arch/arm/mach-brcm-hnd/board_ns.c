...
#define FLASH_PARTS_NUM	(6+MTD_PARTS+PLC_PARTS+FAILSAFE_PARTS+CRASHLOG_PARTS)

static struct mtd_partition bcm947xx_flash_parts[] = 
{
	{
		.name = "boot",
		.size = SZ_256K,
	...
	{
		.name = "squashfs",
		.size = SZ_8M + SZ_1M - SZ_256K - SZ_128K - SZ_64K,
		.offset = MTDPART_OFS_NXTBLK,
	},
	{
                .name = "crash",
...
//Roy +++
		...
		bcm947xx_flash_parts[nparts].size = mtd->size - vmlz_off;
#else
		bcm947xx_flash_parts[nparts].size = rfs_off - vmlz_off;
#endif
//Roy ---
...
//Roy +++
		/* Reserve for BOARD_DATA AND NVRAM */
//		bcm947xx_flash_parts[nparts].size -= 2*ROUNDUP(nvram_space, mtd->erasesize);
//Roy ---
...
#else
//		bcm947xx_flash_parts[nparts].size = mtd->size - rfs_off - 2 * 0x10000;
		bcm947xx_flash_parts[nparts].size = 0xEE0000 - rfs_off; 
#endif
...
//Roy +++
//Create a rootfs data partition
		nparts++;
//Roy ---
    ...
    /*Setup board data MTD partition*/
    bcm947xx_flash_parts[nparts].name = "board_data";
	bcm947xx_flash_parts[nparts].size = ROUNDUP(nvram_space, mtd->erasesize);
	bcm947xx_flash_parts[nparts].offset = size - 2*bcm947xx_flash_parts[nparts].size;
	nparts++;
